//
// Rule: Link multiple accounts using the same email address (e.g. classic email password auth and google OAuth)
//
...
//
// Enrich profile with FullContact data using email address
//

function(user, context, callback) {
	const FULLCONTACT_KEY = configuration.FULLCONTACT_KEY;
	const namespace = 'https://auth0.chatdocs.io/';
	console.log('[/] Starting enrichment');
	console.log('[/] Executing for email :', user.email);
	// We skip if no email
	...
	request.post('https://api.fullcontact.com/v3/person.enrich', {
		...
		//console.log('[/] FC Reponse:', response);
		//console.log('[/] FC Body:', body);
		if (error || (response && response.statusCode !== 200)) {
			// Swallow fullcontact API errors and just continue login
			console.log('[/] Error in request. Error:', error);
			return callback(null, user, context);
		}
		...
		console.log('[/] Enrichment looks good! Body:');
		console.log(body);

		auth0.users.updateUserMetadata(user.user_id, user.user_metadata);
		context.idToken[namespace + 'fullcontact'] = user.user_metadata.fullcontact;
...
//
// Rule: Infer gender from first name
//

function(user, context, callback) {
	const namespace = 'https://auth0.chatdocs.io/';
	console.log('[/] Starting gender enrichment');
	...
	// We skip if no given name
	if (!user.given_name) return callback(null, user, context);

	console.log('[/] Executing for given name:', user.given_name);
	console.log('[/] Cleaned name (with closest ASCII chars):', cleanStr(user.given_name));

	// skip if given name metadata is already there
	if (user.user_metadata && user.user_metadata.gender) return callback(null, user, context);

	request.get('https://api.genderize.io/?name=' + cleanStr(user.given_name), {}, (error, response, body) => {
		//console.log('[/] FC Reponse:', response);
		...
		// if we reach here, it means our gender API returned info and we'll add it to the metadata
		user.user_metadata = user.user_metadata || {};
		user.user_metadata.gender = body;
		console.log('[/] Enrichment looks good! Gender should be in body:');
		console.log(body);
